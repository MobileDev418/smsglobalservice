...
$q="select spid, fname, lname , clientname, clientmobile, comments, corres_dt from clients_correspondents, customer where ".$wherespq."  corres_dt >='".$lower."' and corres_dt <='".$upper."' and clients_correspondents.spid=customer.cus_id  order by corres_dt desc";


//Get the total records to do paging

$_SESSION['total_cus_his_rec']=$db->get_var("select count(*) from clients_correspondents where ".$wherespq." corres_dt >='".$lower."' and corres_dt <='".$upper."'");
...
if($_POST['exporttoexcel']==1) //Check that admin requested for excel report
		{
			...
			include_once "export_report_Salesperson_client_Correspondents.php";
		}
...
<html>
<head>
<title><?php echo $ADMIN_TITLE?> - Client Correspondents History</title>
...
<tr>
    <td class="crown_head"><?php echo $WEBSITE ?> - Client Correspondents History</td>
  </tr>
<tr>
  <?php
...
<tr class="topheading"><td colspan="6">Client Correspondents</td></tr>
<tr class="topheading" style="font-size:11px; font-weight:normal">
	<th width="35" align="left">Sr#</th>
	<th width="100" align="left">Sales Person</th>
	<th width="100" align="left" >Client Name</th>
    <th width="85" align="left" >Client Mobile</th>
    <th width="224" align="left" >Comments</th>
...
if($sms_history)
{
	foreach($sms_history as $history)
	{
		...
		echo "<td align='center'>".$ind."</td>";
    		...
    		echo "<td align='left'>".$history->clientname."</td>";
		echo "<td align='left'>".$history->clientmobile."</td>";
    		echo "<td align='left'>".$history->comments."</td>";
    		echo "<td align='right'>".date('d-m-Y',strtotime($history->corres_dt))."</td>";
		echo "</tr>";

	}
}	
?>
<tr><td colspan="6" align="right"><a href="correspondents.php">Back</a></td></tr>
</table>  
