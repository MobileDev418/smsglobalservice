...
<?php
    // Get data from bulk sms form
	// File is included in sms_bulk.php file
	// saves uploaded bulk data from temporary table to sms_q
	...
	$phones=$db->get_results("select distinct(mobile) as mob from ".$table);
 	...
 	if($phones)
		{
			$i=0;
			foreach($phones as $phone) //Loop for each mobile number in temporaray table
			{
				...
				//Query to insert message in sms_q
