...
//File is called from history.php. Get date interval to dipslay sms history. This page shows only those messages which were send successfully
...
$q="select * from history where sms_date >='".$lower."' and sms_date <='".$upper."' order by sms_date desc";

//Get the total records to do paging

$_SESSION['total_his_rec']=$db->get_var("select count(*) from history where sms_date >='".$lower."' and sms_date <='".$upper."'");

//Divide total records to 20 to get total pages

$totalpages= ceil($_SESSION['total_his_rec']/20);
...
if($_POST['exporttoexcel']==1) //Check that admin requested for excel report
		{
			...
			include_once "export_report_smshistroy.php";
		}
...
<table align="center" cellpadding="2" cellspacing="2" class="admin_table">
    ...
    <td style="text-align:right; padding-top:8px;" colspan="8"><span style="color:#900">[Download in bunches of 500 ]</span> Page #&nbsp;
    	...
    	<tr>
        <td width="250px" style="font-size:10px;"><span style="font-weight:bold"><?php echo $_SESSION['total_his_rec'];  ?></span> Record(s) Found - Page <span style="font-weight:bold"><?php echo $page_no;?></span> of <span style="font-weight:bold"><?php echo $totalpages;?></td>
...
<tr class="topheading"><td colspan="6"> Sms History</td>
</tr>
<tr class="topheading" style="font-size:11px; font-weight:normal">
	<th width="35" align="left">Sr#</th>
	<th width="324" align="left" >SMS</th>
    <th width="85" align="left" >To</th>
    <th width="89" align="left" >Phone</th>
    <th width="85" align="left" >From</th>
     <th width="108" align="center">Date & Time</th>
</tr>
...
if($sms_history)
{
	foreach($sms_history as $history)
	{
    	...
    	echo "<td align='left'><a href='view_customer.php?id=".$history->cus_id."'>".$history->sms_from."</a></td>";
...
?>
<tr><td colspan="6" align="right"><a href="index.php">Back</a></td></tr>
</table>  
