    ...
    <?php
	   /* Ajax file to send bulk messages in q. This file displays contents when bulk sms form is submiited by the customer. and is included in sms_bulk.php file which uploaded the bulk contacts into a temporary table */
		...
		//if sms credits are less than one then simply display message insufficent balance
         ...
         if($credit<1)
         {
                echo '<div style="color:red; text-align:left;font-size:125%;font-weight:bold;margin:5px 0 5px 0px;" id="insufficentbalancediv"><div style="background:#000;color:#ff0000;font-size:20px;padding:10px;text-align:center">Insufficient credits!</div></div>';
		 ...
		 else
		 {
			...
			else
			{
				...
				echo '<div class="notice"><div class="typo-icon">Total Messages to Send: <span style="color:#ff0000">'.$_SESSION['total_mob_send'].'</span><div style="padding:8px 0px; text-align:left" id="surebutton"><br/><pre>Are you sure to send messages?</pre><br/><input type="button" class="formbutton" value="Yes" onclick="startbulk(); AJAXBULK(\''.$ajax_url.'\');" />  <input type="button" class="formbutton" value="No" onclick="window.location=\'bulksms.php\';" /></div></div></div>';
